...
static ft::MapIterator<Key, T> get_n_fwd(ft::Map<Key, T> &map, size_t index)
{
	size_t i = 0;
	for (ft::MapIterator<Key, T> ite = map.begin(); ite != map.end(); ++ite)
	{
		if (i++ == index)
			return ite;
	}
...
static void test_insert()
{
	...
	assert(map.empty());
	res = map.insert(ft::pair<int, int>(13, 150));
	assert(res.second == true);
	assert(map.size() == 1);
	assert(!map.empty());
	// ft::pair<int, int> test = (*map.begin());
	assert((*map.begin()).first == 13);
	res = map.insert(ft::pair<int, int>(13, 130));
	assert(res.second == false);
	assert(map.size() == 1);
	res = map.insert(ft::pair<int, int>(62, 130));
	assert(map.size() == 2);
}
...
{
	ft::pair<int, int> array[5] = {
		ft::pair<int, int>(3, 32),
		ft::pair<int, int>(4, 0),
		ft::pair<int, int>(314, 3),
		ft::pair<int, int>(0, 6),
		ft::pair<int, int>(1, 6),
	};
...
static void test_swap()
{
	...
	ft::Map<int, int> map(array, array + 5);
	ft::pair<int, int> array2[3] = {
		ft::pair<int, int>(10, 32),
		ft::pair<int, int>(20, 32),
		ft::pair<int, int>(30, 32),
	};
